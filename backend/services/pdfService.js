...
export const generateQuotationPDF = async (
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:40px;" class="page-break">
    <div style="display:flex;align-items:flex-start;gap:16px;">
      ${
        q.companyLogo
          ? `<img src="${q.companyLogo}" style="width:64px;height:64px;object-fit:contain;border-radius:8px;" />`
          : `<div style="width:64px;height:64px;background:#f1f5f9;border-radius:8px;border:1px solid #e2e8f0;display:flex;align-items:center;justify-content:center;font-size:22px;color:#94a3b8;">🏢</div>`
      }
      <div>
  ...
  <div style="background:#f8fafc;border:1px solid #f1f5f9;border-radius:12px;padding:24px;margin-bottom:40px;" class="page-break">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;border-top:1px solid #f1f5f9;padding-top:40px;" class="page-break">
    ...
    // ── 2. MODERN ────────────────────────────────────────────────────────
    const modernHTML = () => `<!DOCTYPE html><html>${baseHead}<body class="font-sans" style="background:white;padding:0;color:#111827;">
  <div style="height:12px;background:#2563eb;width:100%;"></div>
  <div style="padding:48px;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #f3f4f6;padding-bottom:32px;margin-bottom:32px;" class="page-break">
      <div>
        ${q.companyLogo ? `<img src="${q.companyLogo}" style="height:64px;margin-bottom:16px;object-fit:contain;" />` : ""}
        <h1 style="font-size:32px;font-weight:900;color:#111827;letter-spacing:-.04em;text-transform:uppercase;margin:0 0 8px;">${q.companyName}</h1>
        <p style="font-size:13px;color:#6b7280;margin:2px 0;">${q.companyPhone}</p>
        <p style="font-size:13px;color:#6b7280;margin:2px 0;">${q.companyEmail}</p>
      </div>
      <div style="text-align:right;">
        <h2 style="font-size:36px;font-weight:900;color:rgba(37,99,235,.1);text-transform:uppercase;letter-spacing:.15em;margin:0 0 8px;">Quotation</h2>
        <p style="font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.1em;margin:0 0 4px;">Quote No.</p>
        <p style="font-size:16px;font-weight:700;color:#111827;margin:0 0 8px;">${q.quotationNo}</p>
        <p style="font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.1em;margin:0 0 4px;">Date</p>
        <p style="font-size:13px;font-weight:600;color:#1f2937;margin:0;">${q.date}</p>
      </div>
    </div>
    <div style="background:#f9fafb;border:1px solid #f3f4f6;border-radius:16px;padding:24px;margin-bottom:40px;" class="page-break">
      <p style="font-size:11px;font-weight:900;color:#2563eb;text-transform:uppercase;letter-spacing:.15em;margin:0 0 4px;">Quote For</p>
      <h3 style="font-size:20px;font-weight:700;color:#111827;margin:0 0 4px;">${q.clientName}</h3>
      <p style="font-size:13px;color:#6b7280;margin:0;">${q.clientAddress}</p>
    </div>
    ${buildSectionsHTML({ headerBg: "#2563eb", borderColor: "#f3f4f6", categoryValueColor: "#1d4ed8" })}
    <div style="display:flex;justify-content:flex-end;margin-bottom:48px;" class="page-break">
      <div style="width:300px;background:#111827;color:white;padding:24px;border-radius:24px;">
        <h4 style="font-size:11px;font-weight:900;color:#6b7280;text-transform:uppercase;letter-spacing:.15em;margin:0 0 16px;">Billing Summary</h4>
        <div style="display:flex;justify-content:space-between;font-size:13px;color:#d1d5db;margin-bottom:12px;"><span>Subtotal</span><span style="font-weight:600;">₹ ${q.subtotal}</span></div>
        ${Number(q.discountAmount) > 0 ? `<div style="display:flex;justify-content:space-between;font-size:13px;color:#6ee7b7;margin-bottom:12px;"><span>Discount</span><span style="font-weight:600;">- ₹ ${q.discountAmount}</span></div>` : ""}
        ${Number(q.tax) > 0 ? `<div style="display:flex;justify-content:space-between;font-size:13px;color:#d1d5db;margin-bottom:12px;"><span>Tax</span><span style="font-weight:600;">₹ ${q.tax}</span></div>` : ""}
        <div style="border-top:1px solid #374151;margin:8px 0 16px;"></div>
        <div style="display:flex;justify-content:space-between;align-items:flex-end;">
          <span style="font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.1em;">Grand Total</span>
          <span style="font-size:28px;font-weight:900;color:#34d399;">₹ ${q.grandTotal}</span>
        </div>
      </div>
    </div>
    <div style="margin-top:32px;" class="page-break">
      <h4 style="font-size:11px;font-weight:900;color:#9ca3af;text-transform:uppercase;letter-spacing:.15em;margin:0 0 12px;">Terms &amp; Conditions</h4>
      <ol style="padding-left:16px;margin:0;">
        ${q.terms.map((t) => `<li style="font-size:13px;color:#6b7280;margin-bottom:6px;line-height:1.6;">${t}</li>`).join("")}
      </ol>
    </div>
  </div>
</body></html>`;
  ...
  <div style="border:1px solid #cbd5e1;padding:24px;margin-bottom:40px;background:#f8fafc;position:relative;" class="page-break">
  ...
  <div style="display:grid;grid-template-columns:7fr 5fr;gap:32px;border-top:2px solid #f1f5f9;padding-top:32px;font-family:Inter,sans-serif;" class="page-break">
    ...
    // ── 4. COMPACT ───────────────────────────────────────────────────────
    const compactHTML = () => `<!DOCTYPE html><html>${baseHead}<body class="font-sans" style="background:white;padding:24px 32px;color:#111827;border-top:8px solid #059669;">
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;border-bottom:2px solid #059669;padding-bottom:16px;margin-bottom:16px;" class="page-break">
    <div style="display:flex;align-items:center;gap:12px;">
      ${q.companyLogo ? `<img src="${q.companyLogo}" style="width:48px;height:48px;object-fit:contain;" />` : ""}
      <div>
        <h1 style="font-size:22px;font-weight:900;color:#065f46;text-transform:uppercase;margin:0 0 4px;">${q.companyName}</h1>
        <p style="font-size:11px;color:#374151;margin:1px 0;">${q.companyPhone}</p>
        <p style="font-size:11px;color:#374151;margin:1px 0;">${q.companyEmail}</p>
      </div>
    </div>
    <div style="text-align:right;">
      <h2 style="font-size:22px;font-weight:900;color:#1f2937;text-transform:uppercase;letter-spacing:.15em;margin:0 0 8px;">QUOTATION</h2>
      <table style="margin-left:auto;font-size:11px;">
        <tr><td style="font-weight:700;color:#6b7280;text-transform:uppercase;padding-right:16px;text-align:right;">Ref:</td><td style="font-weight:700;color:#111827;">${q.quotationNo}</td></tr>
        <tr><td style="font-weight:700;color:#6b7280;text-transform:uppercase;padding-right:16px;text-align:right;">Date:</td><td style="font-weight:700;color:#111827;">${q.date}</td></tr>
      </table>
    </div>
  </div>
  <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:12px;margin-bottom:24px;" class="page-break">
    <p style="font-size:10px;font-weight:700;color:#059669;text-transform:uppercase;letter-spacing:.15em;margin:0 0 4px;">Bill To:</p>
    <h3 style="font-size:14px;font-weight:700;color:#111827;margin:0 0 2px;">${q.clientName}</h3>
    <p style="font-size:12px;color:#6b7280;margin:0;">${q.clientAddress}</p>
  </div>
  ${q.sections.map((sec) => `
    <div class="page-break" style="margin-bottom:24px;">
      <h4 style="font-size:11px;font-weight:700;color:#1f2937;text-transform:uppercase;background:#f3f4f6;border:1px solid #d1d5db;border-bottom:none;padding:6px 12px;border-radius:6px 6px 0 0;margin:0;">${sec.title}</h4>
      <table style="width:100%;border-collapse:collapse;border:1px solid #d1d5db;">
        <thead><tr style="background:#059669;color:white;font-size:11px;text-transform:uppercase;letter-spacing:.05em;">
          <th style="padding:8px 12px;border:1px solid #047857;width:36px;text-align:center;">#</th>
          <th style="padding:8px 12px;border:1px solid #047857;text-align:left;">Description</th>
          <th style="padding:8px 12px;border:1px solid #047857;text-align:center;width:70px;">Labour</th>
          <th style="padding:8px 12px;border:1px solid #047857;text-align:center;width:70px;">Material</th>
          <th style="padding:8px 12px;border:1px solid #047857;text-align:right;width:90px;">Total</th>
        </tr></thead>
        <tbody>
          ${sec.items.map((item, i) => `
            <tr style="border-bottom:1px solid #e5e7eb;background:${i % 2 === 1 ? "#f9fafb" : "white"};">
              <td style="padding:8px 12px;border-right:1px solid #d1d5db;text-align:center;font-size:11px;color:#6b7280;font-weight:600;">${i + 1}</td>
              <td style="padding:8px 12px;border-right:1px solid #d1d5db;font-size:12px;font-weight:600;color:#1f2937;white-space:pre-wrap;">${item.desc}</td>
              <td style="padding:8px 12px;border-right:1px solid #d1d5db;font-size:12px;text-align:center;color:#4b5563;">${item.labour}</td>
              <td style="padding:8px 12px;border-right:1px solid #d1d5db;font-size:12px;text-align:center;color:#4b5563;">${item.material}</td>
              <td style="padding:8px 12px;font-size:12px;font-weight:700;color:#111827;text-align:right;">₹ ${item.total}</td>
            </tr>`).join("")}
          <tr style="background:#f9fafb;border-top:2px solid #d1d5db;">
            <td colspan="4" style="padding:8px 12px;text-align:right;font-size:11px;font-weight:700;color:#4b5563;text-transform:uppercase;letter-spacing:.1em;border-right:1px solid #d1d5db;">Category Total</td>
            <td style="padding:8px 12px;text-align:right;font-weight:700;color:#065f46;">₹ ${sec.sectionTotal}</td>
          </tr>
        </tbody>
      </table>
    </div>`).join("")}
  <div style="display:flex;gap:16px;margin-bottom:24px;" class="page-break">
    <div style="flex:1;border:1px solid #d1d5db;border-radius:8px;padding:12px;">
      <h4 style="font-size:11px;font-weight:700;color:#065f46;text-transform:uppercase;letter-spacing:.1em;border-bottom:1px solid #e5e7eb;padding-bottom:6px;margin:0 0 10px;">Bank Details</h4>
      <table style="font-size:12px;width:100%;">
        ${[["Bank:", q.bankName], ["A/C No:", q.accNo], ["IFSC:", q.ifsc], ...(q.accHolder ? [["Holder:", q.accHolder]] : [])].map(([l, v]) => `<tr><td style="color:#6b7280;padding-bottom:4px;width:80px;">${l}</td><td style="font-weight:700;color:#1f2937;padding-bottom:4px;">${v}</td></tr>`).join("")}
      </table>
    </div>
    <div style="width:240px;border:1px solid #059669;border-radius:8px;overflow:hidden;display:flex;flex-direction:column;">
      <div style="padding:12px;background:#f9fafb;flex:1;">
        <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:6px;"><span style="font-weight:700;color:#4b5563;text-transform:uppercase;">Subtotal</span><span style="font-weight:700;color:#1f2937;">₹ ${q.subtotal}</span></div>
        ${Number(q.discountAmount) > 0 ? `<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:6px;"><span style="font-weight:700;color:#4b5563;text-transform:uppercase;">Discount</span><span style="font-weight:700;color:#059669;">- ₹ ${q.discountAmount}</span></div>` : ""}
        ${Number(q.tax) > 0 ? `<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:6px;"><span style="font-weight:700;color:#4b5563;text-transform:uppercase;">Tax</span><span style="font-weight:700;color:#1f2937;">₹ ${q.tax}</span></div>` : ""}
      </div>
      <div style="background:#059669;padding:12px;color:white;display:flex;justify-content:space-between;align-items:center;">
        <span style="font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.1em;">Total</span>
        <span style="font-size:18px;font-weight:900;">₹ ${q.grandTotal}</span>
      </div>
    </div>
  </div>
  <div style="border-top:1px solid #e5e7eb;padding-top:16px;" class="page-break">
    <h4 style="font-size:10px;font-weight:900;color:#9ca3af;text-transform:uppercase;letter-spacing:.15em;margin:0 0 8px;">Terms &amp; Conditions</h4>
    <ul style="padding-left:16px;margin:0;">${q.terms.map((t) => `<li style="font-size:11px;color:#4b5563;margin-bottom:4px;line-height:1.5;">${t}</li>`).join("")}</ul>
  </div>
</body></html>`;

    // ── 5. CREATIVE ──────────────────────────────────────────────────────
    const creativeHTML = () => `<!DOCTYPE html><html>${baseHead}<body class="font-sans" style="background:white;padding:0;color:#111827;">
  <div style="display:flex;min-height:200px;">
    <div style="background:#111827;color:white;flex:1;padding:40px;border-bottom-right-radius:80px;">
      <div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
        ${q.companyLogo ? `<img src="${q.companyLogo}" style="width:56px;height:56px;object-fit:contain;border-radius:16px;background:white;padding:4px;" />` : `<div style="width:48px;height:48px;background:linear-gradient(135deg,#a855f7,#ec4899);border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:22px;">🎨</div>`}
        <h1 style="font-size:28px;font-weight:900;letter-spacing:-.02em;margin:0;">${q.companyName}</h1>
      </div>
      <p style="font-size:13px;color:#d1d5db;margin:4px 0;">${q.companyPhone}</p>
      <p style="font-size:13px;color:#d1d5db;margin:4px 0;">${q.companyEmail}</p>
    </div>
    <div style="flex:1;padding:40px;display:flex;flex-direction:column;justify-content:center;align-items:flex-end;text-align:right;">
      <h2 style="font-size:48px;font-weight:900;color:#9333ea;text-transform:uppercase;letter-spacing:-.04em;margin:0 0 16px;">Estimate</h2>
      <p style="font-size:18px;font-weight:900;color:#111827;margin:0 0 12px;">${q.quotationNo}</p>
      <p style="font-size:13px;font-weight:700;color:#1f2937;margin:0;">${q.date}</p>
    </div>
  </div>
  <div style="padding:24px 40px 40px;">
    <div style="background:white;border:1px solid #f3f4f6;border-radius:24px;padding:24px;display:inline-block;min-width:280px;margin-bottom:40px;box-shadow:0 4px 20px rgba(0,0,0,.04);" class="page-break">
      <p style="font-size:11px;font-weight:900;color:#a855f7;text-transform:uppercase;letter-spacing:.15em;margin:0 0 12px;">Prepared For</p>
      <h3 style="font-size:24px;font-weight:900;color:#111827;margin:0 0 8px;">${q.clientName}</h3>
      <p style="font-size:13px;color:#6b7280;margin:0;">${q.clientAddress}</p>
    </div>
    ${q.sections.map((sec) => `
      <div class="page-break" style="margin-bottom:40px;">
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:16px;">
          <span style="font-size:12px;font-weight:900;color:#7e22ce;text-transform:uppercase;letter-spacing:.15em;background:#faf5ff;padding:8px 16px;border-radius:12px;">${sec.title}</span>
          <div style="flex:1;border-bottom:1px dashed #e5e7eb;"></div>
        </div>
        <table style="width:100%;border-collapse:collapse;">
          <thead><tr style="border-bottom:2px solid #111827;">
            <th style="padding:16px 8px;font-size:11px;font-weight:900;color:#111827;text-transform:uppercase;letter-spacing:.15em;text-align:left;">Description</th>
            <th style="padding:16px 8px;font-size:11px;font-weight:900;color:#111827;text-transform:uppercase;letter-spacing:.15em;text-align:center;width:90px;">Labour</th>
            <th style="padding:16px 8px;font-size:11px;font-weight:900;color:#111827;text-transform:uppercase;letter-spacing:.15em;text-align:center;width:90px;">Material</th>
            <th style="padding:16px 8px;font-size:11px;font-weight:900;color:#111827;text-transform:uppercase;letter-spacing:.15em;text-align:right;width:110px;">Total</th>
          </tr></thead>
          <tbody>
            ${sec.items.map((item) => `
              <tr style="border-bottom:1px dashed #f3f4f6;">
                <td style="padding:20px 8px;font-size:13px;font-weight:700;color:#1f2937;white-space:pre-wrap;">${item.desc}</td>
                <td style="padding:20px 8px;font-size:13px;color:#6b7280;text-align:center;">${item.labour}</td>
                <td style="padding:20px 8px;font-size:13px;color:#6b7280;text-align:center;">${item.material}</td>
                <td style="padding:20px 8px;font-size:15px;font-weight:900;color:#111827;text-align:right;">₹ ${item.total}</td>
              </tr>`).join("")}
            <tr style="background:#fafafa;">
              <td colspan="3" style="padding:16px 8px;text-align:right;font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.15em;">Category Total</td>
              <td style="padding:16px 8px;text-align:right;font-size:15px;font-weight:700;color:#9333ea;">₹ ${sec.sectionTotal}</td>
            </tr>
          </tbody>
        </table>
      </div>`).join("")}
    <div style="display:flex;justify-content:flex-end;margin-bottom:48px;" class="page-break">
      <div style="width:360px;">
        ${[["Subtotal", `₹ ${q.subtotal}`, "#6b7280", "#1f2937"], ...(Number(q.discountAmount) > 0 ? [["Discount", `- ₹ ${q.discountAmount}`, "#6b7280", "#10b981"]] : []), ...(Number(q.tax) > 0 ? [["Tax", `₹ ${q.tax}`, "#6b7280", "#1f2937"]] : [])].map(([l, v, lc, vc]) => `<div style="display:flex;justify-content:space-between;padding:0 8px;margin-bottom:10px;"><span style="color:${lc};font-size:13px;font-weight:700;text-transform:uppercase;">${l}</span><span style="color:${vc};font-weight:700;">${v}</span></div>`).join("")}
        <div style="background:linear-gradient(90deg,#9333ea,#ec4899);border-radius:24px;padding:24px;color:white;display:flex;justify-content:space-between;align-items:center;margin-top:16px;">
          <span style="font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.15em;">Total Payable</span>
          <span style="font-size:28px;font-weight:900;">₹ ${q.grandTotal}</span>
        </div>
      </div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;" class="page-break">
      <div>
        <h4 style="font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.15em;margin:0 0 16px;">Terms &amp; Conditions</h4>
        ${q.terms.map((t, i) => `<div style="display:flex;gap:8px;font-size:13px;color:#6b7280;margin-bottom:10px;line-height:1.6;"><span style="color:#c084fc;font-weight:900;">${i + 1}.</span><span>${t}</span></div>`).join("")}
      </div>
      <div style="background:#fafafa;border-radius:24px;padding:24px;border:1px solid #f3f4f6;">
        <h4 style="font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.15em;margin:0 0 16px;">Bank Details</h4>
        ${[["Bank", q.bankName], ["A/C No.", q.accNo], ["IFSC", q.ifsc], ...(q.accHolder ? [["Name", q.accHolder]] : [])].map(([l, v]) => `<div style="display:flex;justify-content:space-between;border-bottom:1px dashed #e5e7eb;padding-bottom:8px;margin-bottom:8px;font-size:13px;"><span style="color:#6b7280;">${l}</span><span style="font-weight:700;color:#111827;">${v}</span></div>`).join("")}
      </div>
    </div>
  </div>
</body></html>`;
    ...
    // ── 6. GROUPED ───────────────────────────────────────────────────────
    const groupedHTML = () => `<!DOCTYPE html><html>${baseHead}<body class="font-serif" style="background:white;padding:48px;color:#111827;border:1px solid #f3f4f6;">
  <div style="text-align:center;border-bottom:2px solid #1f2937;padding-bottom:24px;margin-bottom:32px;" class="page-break">
    ${q.companyLogo ? `<img src="${q.companyLogo}" style="height:64px;margin:0 auto 12px;display:block;object-fit:contain;" />` : ""}
    <h1 style="font-size:28px;font-weight:900;text-transform:uppercase;letter-spacing:.08em;color:#111827;margin:0 0 8px;">${q.companyName}</h1>
    <p style="font-size:13px;font-family:Inter,sans-serif;color:#4b5563;margin:0;">${q.companyPhone} | ${q.companyEmail}</p>
  </div>
  <div style="font-family:Inter,sans-serif;margin-bottom:32px;" class="page-break">
    <div style="display:flex;justify-content:space-between;margin-bottom:24px;">
      <p style="font-weight:700;margin:0;">Date: ${q.date}</p>
      <p style="font-weight:700;color:#6b7280;margin:0;">Ref: ${q.quotationNo}</p>
    </div>
    <p style="font-weight:700;text-decoration:underline;text-underline-offset:4px;margin:0 0 16px;">Subject: Quotation for ${q.clientName}, ${q.clientAddress}</p>
    <p style="font-size:13px;color:#374151;margin:0;">Thank you for your enquiry. Please find below our quotation for Material &amp; Labour for this site.</p>
  </div>
  ${q.sections.map((sec) => `
    <div style="margin-bottom:48px;">
      <h2 style="font-size:17px;font-weight:900;text-transform:uppercase;color:#111827;border-bottom:2px solid #1f2937;padding-bottom:8px;margin-bottom:24px;font-family:Inter,sans-serif;">${sec.title}</h2>
      ${sec.items.map((item) => `
        <div class="page-break" style="margin-bottom:32px;">
          <h3 style="font-size:13px;font-weight:700;color:#111827;border-left:4px solid #1f2937;padding:6px 12px;background:#f9fafb;margin:0;font-family:Inter,sans-serif;white-space:pre-wrap;">${item.desc}</h3>
          <table style="width:100%;border-collapse:collapse;border:1px solid #d1d5db;font-family:Inter,sans-serif;font-size:13px;">
            <tbody>
              <tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:12px;color:#4b5563;width:66%;"><strong style="color:#1f2937;">Labour Cost</strong></td><td style="padding:12px;text-align:right;color:#1f2937;font-weight:500;">₹ ${item.labour}</td></tr>
              <tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:12px;color:#4b5563;"><strong style="color:#1f2937;">Material Cost</strong></td><td style="padding:12px;text-align:right;color:#1f2937;font-weight:500;">₹ ${item.material}</td></tr>
              <tr style="background:#f9fafb;"><td style="padding:12px;text-align:right;font-weight:700;color:#1f2937;font-style:italic;">Total =</td><td style="padding:12px;text-align:right;font-weight:700;color:#111827;">₹ ${item.total}</td></tr>
            </tbody>
          </table>
        </div>`).join("")}
      <div style="text-align:right;font-family:Inter,sans-serif;"><p style="font-size:13px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.1em;margin:0;">${sec.title} Total: <span style="color:#111827;margin-left:16px;">₹ ${sec.sectionTotal}</span></p></div>
    </div>`).join("")}
  <div style="text-align:right;margin-bottom:40px;font-family:Inter,sans-serif;" class="page-break">
    ${Number(q.discountAmount) > 0 ? `<p style="font-size:13px;color:#15803d;font-weight:700;margin:0 0 4px;">Discount: - ₹ ${q.discountAmount}</p>` : ""}
    ${Number(q.tax) > 0 ? `<p style="font-size:13px;color:#4b5563;margin:0 0 4px;">Tax: ₹ ${q.tax}</p>` : ""}
    <h2 style="font-size:19px;font-weight:700;color:#111827;border-top:2px solid #1f2937;border-bottom:2px solid #1f2937;padding:12px 16px 12px 32px;display:inline-block;background:#f9fafb;">Total Working Cost = ₹ ${q.grandTotal}</h2>
  </div>
  <div style="display:flex;justify-content:space-between;align-items:flex-end;border-top:1px solid #e5e7eb;padding-top:32px;font-family:Inter,sans-serif;" class="page-break">
    <div>
      <h4 style="font-size:13px;font-weight:700;color:#111827;margin:0 0 8px;">Bank Details:</h4>
      ${[["Bank:", q.bankName], ["A/c No:", q.accNo], ["IFSC:", q.ifsc], ...(q.accHolder ? [["Name:", q.accHolder]] : [])].map(([l, v]) => `<p style="font-size:13px;color:#374151;margin:4px 0;"><strong style="color:#1f2937;display:inline-block;width:96px;">${l}</strong>${v}</p>`).join("")}
    </div>
    <div style="text-align:right;">
      <p style="font-size:13px;color:#6b7280;font-style:italic;margin:0 0 48px;">With Regards,</p>
      <h4 style="font-size:17px;font-weight:700;color:#111827;margin:0 0 4px;">${q.companyName}</h4>
      <p style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:.15em;margin:0;">Authorized Signatory</p>
    </div>
  </div>
</body></html>`;
  ...
  <div style="position:relative;z-index:1;">
    ...
    <div style="background:#0c1535;position:relative;overflow:hidden;">
      ...
      <div style="background:#111829;padding:14px 52px;display:flex;gap:48px;align-items:center;">
        ...
        <div style="width:1px;height:20px;background:rgba(255,255,255,.08);"></div>
        <div style="display:flex;align-items:center;gap:12px;">
          <span style="font-size:9px;font-weight:600;color:rgba(255,255,255,.28);text-transform:uppercase;letter-spacing:.22em;">Status</span>
          <span style="font-size:12px;font-weight:500;color:#e74c3c;">Awaiting Approval</span>
        </div>
      </div>
    </div>
