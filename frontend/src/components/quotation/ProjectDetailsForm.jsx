...
import { Building2, Ruler, FileText, Image as ImageIcon, X } from "lucide-react";
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div className={cardStyle + " mb-0"}>
          <div className={headerStyle}>
            <div className="p-2.5 bg-emerald-50 rounded-xl text-emerald-600"><Ruler size={22}/></div>
            <div>
              <h2 className="text-xl font-bold text-slate-900 leading-none">Area Details</h2>
            </div>
          </div>
          <div className="space-y-6">
            <div><label className={labelStyle}>Interior Area (Sqft)</label><input type="number" className={inputStyle} placeholder="0" value={formData.areaDetails.interiorArea} onChange={(e) => handleNestedChange("areaDetails", "interiorArea", e.target.value)} /></div>
            <div><label className={labelStyle}>Exterior Area (Sqft)</label><input type="number" className={inputStyle} placeholder="0" value={formData.areaDetails.exteriorArea} onChange={(e) => handleNestedChange("areaDetails", "exteriorArea", e.target.value)} /></div>
          </div>
        </div>

        <div className={cardStyle + " mb-0"}>
          <div className={headerStyle}>
            <div className="p-2.5 bg-amber-50 rounded-xl text-amber-600"><FileText size={22}/></div>
            <div>
              <h2 className="text-xl font-bold text-slate-900 leading-none">Cover Letter</h2>
            </div>
          </div>
          <div className="space-y-6">
            ...
            <div><label className={labelStyle}>Body</label><textarea rows={3} className={`${inputStyle} resize-none`} value={formData.coverLetter.body} onChange={(e) => handleNestedChange("coverLetter", "body", e.target.value)} /></div>
          </div>
        </div>
      </div>
    </>
