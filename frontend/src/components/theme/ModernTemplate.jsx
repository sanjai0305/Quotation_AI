...
export default function ModernTemplate({ data }) {
  ...
  return (
    <div className="bg-white p-12 min-h-[297mm] text-gray-800 font-sans relative">
      ...
      {/* 🟦 TOP ACCENT BAR */}
      <div className="absolute top-0 left-0 w-full h-3 bg-blue-600 print:bg-blue-600"></div>

      {/* 🏢 HEADER SECTION */}
      <div className="flex justify-between items-start border-b-2 border-gray-100 pb-8 mb-8 mt-4 print-avoid-break">
        <div>
          {quote.companyLogo && (
            <img src={quote.companyLogo} alt="Logo" className="h-16 mb-4 object-contain" />
          )}
        ...
        <div className="text-right">
          <h2 className="text-4xl font-black text-blue-600/10 tracking-widest uppercase print:text-blue-100">Quotation</h2>
          <div className="mt-2">
      ...
      <div className="mb-10 bg-gray-50 p-6 rounded-2xl border border-gray-100 flex justify-between items-center print-avoid-break">
        <div>
      ...
      <div className="mb-10 space-y-8">
        {sectionsToRender.map((section, secIdx) => (
          <div key={secIdx} className="print-avoid-break">
            ...
            <h4 className="text-sm font-bold text-blue-600 uppercase tracking-widest mb-3">
              {section.title}
            </h4>
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden shadow-sm">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-blue-600 text-white text-xs uppercase tracking-widest print:bg-blue-600 print:text-white">
                    <th className="py-4 px-5 font-bold w-1/2">Description of Work</th>
                    <th className="py-4 px-5 font-bold text-center">Labour</th>
                    <th className="py-4 px-5 font-bold text-center">Material</th>
                    <th className="py-4 px-5 font-bold text-right">Total/Sqft</th>
                  </tr>
                </thead>
                <tbody>
                  {section.items && section.items.map((item, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                      <td className="py-4 px-5 text-sm font-semibold text-gray-800 whitespace-pre-wrap">{item.desc}</td>
                      <td className="py-4 px-5 text-sm text-gray-600 text-center">{safeFormat(item.labour)}</td>
                      <td className="py-4 px-5 text-sm text-gray-600 text-center">{safeFormat(item.material)}</td>
                      <td className="py-4 px-5 text-sm font-bold text-gray-900 text-right">Rs. {safeFormat(item.total)}</td>
                    </tr>
                  ))}
                  
                  {/* Category Total Row */}
                  <tr className="bg-gray-50/80">
                    <td colSpan="3" className="py-3 px-5 text-right text-xs font-bold text-gray-500 uppercase tracking-widest">
                      Category Total
                    </td>
                    <td className="py-3 px-5 text-right text-sm font-bold text-blue-700">
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start mb-12 print-avoid-break">
        
        {/* Left: Bank Details */}
        <div className="flex-1 w-full bg-blue-50 border border-blue-100 p-6 rounded-2xl print:bg-blue-50">
          ...
          <div className="space-y-2 text-sm">
            ...
            {quote.bankDetails?.accHolder && (
              <div className="flex justify-between pt-1">
                <span className="text-blue-800 font-medium">Holder Name</span>
        ...
        <div className="w-full md:w-80 bg-gray-900 text-white p-6 rounded-3xl shadow-xl print:bg-gray-900 print:text-white print:shadow-none">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="font-semibold">Rs. {safeFormat(quote.subtotal)}</span>
          </div>
          
          {Number(quote.discount) > 0 && (
            <div className="flex justify-between items-center mb-3">
              <span className="text-gray-300 text-sm">Discount</span>
              <span className="font-semibold text-emerald-400">- Rs. {safeFormat(quote.discount)}</span>
            </div>
          )}
          ...
          {Number(quote.tax) > 0 && (
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="font-semibold">Rs. {safeFormat(quote.tax)}</span>
            </div>
          )}
          ...
          <div className="border-t border-gray-700 mt-2 pt-4 flex justify-between items-end">
            ...
            <span className="text-3xl font-black text-emerald-400">Rs. {safeFormat(quote.grandTotal)}</span>
          </div>
        </div>
      ...
      {/* 🟦 BOTTOM ACCENT BAR */}
      <div className="absolute bottom-0 right-0 w-1/3 h-2 bg-blue-600 rounded-tl-full print:bg-blue-600"></div>
