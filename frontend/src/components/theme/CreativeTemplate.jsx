...
export default function CreativeTemplate({ data }) {
  ...
  return (
    <div className="bg-white min-h-[297mm] text-gray-800 font-sans relative overflow-hidden">
      ...
      <div className="absolute top-[-150px] right-[-150px] w-[500px] h-[500px] bg-gradient-to-br from-purple-400 to-fuchsia-400 rounded-full opacity-10 blur-[80px] pointer-events-none"></div>
      <div className="absolute bottom-[-100px] left-[-100px] w-[300px] h-[300px] bg-gradient-to-tr from-blue-400 to-emerald-400 rounded-full opacity-10 blur-[60px] pointer-events-none"></div>

      {/* 🔮 TOP HEADER (Split Style) */}
      <div className="flex flex-col md:flex-row min-h-[220px]">
        
        {/* Left: Branding Block */}
        <div className="bg-gray-900 text-white p-10 flex-1 rounded-br-[80px] relative overflow-hidden shadow-xl z-10">
          <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-2xl -mr-10 -mt-10"></div>
          
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-fuchsia-500 rounded-2xl flex items-center justify-center text-white shadow-lg">
               <Palette size={24} />
            </div>
            <h1 className="text-3xl font-black tracking-tight">{quote.companyName}</h1>
          </div>

          <div className="space-y-1.5 text-gray-400 text-sm font-medium">
        ...
        <div className="p-10 flex-1 flex flex-col justify-center items-end text-right z-10">
          <h2 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-fuchsia-600 tracking-tighter uppercase mb-4">
            Estimate
          </h2>
      ...
      <div className="px-10 pb-10 pt-4 relative z-10">
        ...
        <div className="bg-white/60 backdrop-blur-sm border border-gray-100 p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-10 inline-block min-w-[300px]">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr>
                ...
                <th className="py-4 px-2 border-b-2 border-gray-900 text-xs font-black text-gray-900 uppercase tracking-widest text-center">Labour</th>
                <th className="py-4 px-2 border-b-2 border-gray-900 text-xs font-black text-gray-900 uppercase tracking-widest text-center">Material</th>
                <th className="py-4 px-2 border-b-2 border-gray-900 text-xs font-black text-gray-900 uppercase tracking-widest text-right">Total</th>
              </tr>
            </thead>
            <tbody>
              {quote.items.map((item, index) => (
                <tr key={index} className="border-b border-gray-100 border-dashed hover:bg-gray-50/50 transition-colors">
                  <td className="py-5 px-2 text-sm font-bold text-gray-800">{item.desc}</td>
        ...
        <div className="flex justify-end mb-12">
          <div className="w-full md:w-96">
            ...
            <div className="mt-4 p-6 bg-gradient-to-r from-purple-600 to-fuchsia-600 rounded-3xl text-white shadow-xl shadow-purple-500/20 flex justify-between items-center transform transition-transform hover:scale-[1.02]">
            ...
            <ul className="text-sm text-gray-500 space-y-2.5 font-medium leading-relaxed">
              {quote.terms.map((term, i) => (
                 <li key={i} className="flex gap-2">
                   <span className="text-purple-400 font-black">{i+1}.</span> {term}
                 </li>
              ))}
            </ul>
          </div>
